...
func (s *Service) ListByAudit(ctx context.Context, auditID uuid.UUID, auditorID *uuid.UUID, procedure string, status string) ([]Finding, error) {
	findings, err := s.repo.ListByAudit(ctx, auditID, auditorID, procedure, status)
	if err != nil {
		return nil, err
	}
	if findings == nil {
